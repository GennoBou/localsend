	...
	for _, ip := range targetIPs {
		for port := protocol.DefaultPort; port <= protocol.DefaultPort+9; port++ {
			...
			go func(targetIP string, targetPort int) {
				...
				scanHost(ctx, client, reqBody, targetIP, targetPort, onDiscover)
			}(ip, port)
		}
	}
	...
	for _, localIP := range localIPs {
		...
		for i := 1; i <= 254; i++ {
			...
			go func(ip string) {
				...
				scanHost(ctx, client, reqBody, ip, protocol.DefaultPort, onDiscover)
			}(targetIP)
		}
	}
...
func scanHost(ctx context.Context, client *http.Client, reqBody []byte, ip string, port int, onDiscover func(protocol.Device)) {
