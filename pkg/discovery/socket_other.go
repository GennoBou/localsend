...
func setMulticastSocketOptions(conn *net.UDPConn, ip net.IP) error {
	...
	err = rawConn.Control(func(fd uintptr) {
		...
		if !ip.Equal(net.IPv4zero) {
			var addr [4]byte
			copy(addr[:], ip4)
			socketErr = unix.SetsockoptInet4Addr(
				int(fd),
				unix.IPPROTO_IP,
				unix.IP_MULTICAST_IF,
				addr,
			)
			if socketErr != nil {
				return
...
func startWindowsMulticastListener(ctx context.Context, addr *net.UDPAddr, interfaces []net.Interface, handleConn func(*net.UDPConn)) error {
	return fmt.Errorf("windows multicast listener is not supported on non-windows platforms")
}
