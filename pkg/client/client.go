...
func (c *Client) SendFiles(ctx context.Context, target *protocol.Device, files []SendFileSource, pin string, progress func(fileID string, sentBytes int64)) ([]SendResult, error) {
	...
	for _, f := range files {
		...
		err := c.uploadSingleFile(ctx, target, sessionID, f, token, progress)
		if err != nil {
...
// uploadSingleFile handles uploading a single file source during an active session.
func (c *Client) uploadSingleFile(ctx context.Context, target *protocol.Device, sessionID string, f SendFileSource, token string, progress func(fileID string, sentBytes int64)) error {
